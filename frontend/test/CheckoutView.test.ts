...
import { CheckoutGateway, CheckoutOutput } from "../src/gateway/CheckoutGateway";
...
test("Deve testar a tela de checkout", async () => {
  ...
  await sleep(200);
  expect(wrapper.get(".output-total").text()).toBe("£6,090.00");
  expect(wrapper.get(".output-freight").text()).toBe("£280.00");
})